...
def create_body_recursive(joint_name, joints, player_name, color, material_name, joint_idx, joint_list, parent_pos=None):
    ...
    ET.SubElement(body, "geom", type="sphere", size=str(joint_defs[idx][1]), material=material_name)
    # Add a joint (if not root)
    if joint_name != "Core":
        ...
        if "Shoulder" in joint_name or "Hip" in joint_name:
            ...
            ET.SubElement(body, "joint", name=joint_name_full, type="hinge", axis="0 1 0", limited="true", range="-2.3 0", damping="0.5")
        else:
    ...
    for child in child_map.get(joint_name, []):
        child_pos = joints[joint_idx[child]]
        rel_child = child_pos - pos
        cap_size = min(joint_defs[idx][1], joint_defs[joint_idx[child]][1])
        if np.linalg.norm(rel_child) > 1e-4:
            ET.SubElement(body, "geom", type="capsule", fromto=f"0 0 0 {rel_child[0]:.4f} {rel_child[1]:.4f} {rel_child[2]:.4f}", size=f"{cap_size:.4f}", material=material_name)
        child_body = create_body_recursive(child, joints, player_name, color, material_name, joint_idx, joint_list, parent_pos=pos)
...
def create_physics_humanoid_xml(joints, player_name, color, material_name, floor_z=-1.0):
    ...
    ET.SubElement(worldbody, "geom", name="floor", type="plane", pos=f"0 0 {floor_z:.4f}", size="30 30 0.1", rgba="0.8 0.8 0.8 1")
...
def create_combined_scene(joint_data, pos_name):
    ...
    compiler = ET.SubElement(mujoco_elem, "compiler", angle="radian", coordinate="local")
    option = ET.SubElement(mujoco_elem, "option", timestep="0.00005", gravity="0 0 -9.81")
    ...
    floor_z = min_z - 0.05
    ET.SubElement(worldbody, "geom", name="ground", type="plane", pos=f"0 0 {floor_z:.4f}", size="5 5 0.1", material="mat_ground", condim="3")
    joint_idx = {name: i for i, (name, _) in enumerate(joint_defs)}
    p1_joints = []
    p2_joints = []
